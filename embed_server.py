...
class E5Model:
    ...
    def _encode(self, input_texts: List[str]) -> np.array:
        batch_dict = self.tokenizer(["query: " + s if not (s.startswith('query: ') or s.startswith('passage: ')) else s for s in input_texts], max_length=512, padding=True, truncation=True, return_tensors='pt')
...
models = {
    'sentence-transformers/all-mpnet-base-v2': load_model('sentence-transformers/all-mpnet-base-v2'),
...
def add_to_cache(cache_backend, transformer, strings, embeddings):
    ...
    for string, embedding in zip(strings, embeddings):
        ...
        virt_file.seek(0)
        pairs.append((transformer.encode('utf-8') + string.encode('utf-8'), virt_file.read()))
    cache_backend.write_batch(pairs)


@app.post("/{transformer:path}")
async def root(transformer, encode_request: EncodeRequest, background_tasks: BackgroundTasks):
    ...
    for index, string in enumerate(encode_request.input):
        cached_data = cache_backend.get(transformer.encode('utf-8') + string.encode('utf-8'))
    ...
    if len(to_embed) > 0:
        ...
        background_tasks.add_task(add_to_cache, cache_backend, transformer, to_embed,
                                  encoded_embeddings)
    virt_file = io.BytesIO()
